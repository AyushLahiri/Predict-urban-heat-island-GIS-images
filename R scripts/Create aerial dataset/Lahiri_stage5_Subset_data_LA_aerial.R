...
###### THIS FILE CREATES THE AERIAL IMAGE DATA SET FOR LA ##########
...
uhi_day = projectRaster(uhi_day,crs = 32611)
uhi_night = projectRaster(uhi_night,crs = 32611)
...
boundary_utm <- st_transform(bg_la, crs = 32611)
city_boundary_utm <- st_transform(la_boundary, crs = 32611)
...
grid_complete_cells_sf <- st_as_sf(regular_df, sf_column_name = "geometry", crs = 32611)
...
write.csv(as.data.frame(cells_filtered) %>% dplyr::select(average,GEOID), 'LA_labels_aeria.csv')
...
for (i in 1:nrow(bbox_data)) {
  ...
  if (status_code(response) == 200) {
    ...
    file_path <- paste("D:/blocks/LA/aerial_images/", geoid, ".png", sep = "")
    print(file_path)
